...
use PhpHelpers\Exceptions\Exception;
/**
 * Description of SFtp
 *
 * @author can.ngo
...
class SFtp {
    ...
    public static function test($server, $user, $password, $port = 21)
    {
        ...
        try
        {
            if(false !== $cid = SFtp::login($server, $user, $password, $port))
            {
                $test = true;
                ftp_close($cid);    
            }
    ...
    public static function rmdir($server, $user, $password, $remote_path, $port = 21)
    {
        ...
        try
        {
            if(false !== $cid = SFtp::login($server, $user, $password, $port))
            {
                ...
                if(SFtp::clean_dir($remote_path, $cid))
                {
                    # If remote_path do not ends with /
                    if(!SString::ends_with($remote_path, '/'))
                    {
    ...
    private static function clean_dir($path, $cid)
    {
        ...
        foreach ($list as $element)
        {
            if($element!=='.' && $element!=='..')
            {
                ...
                if(SFtp::is_dir($element, $cid))
                {
                    # Empty directory
                    SFtp::clean_dir($element, $cid);
    ...
    public static function upload_dir($server, $user, $password, $local_path, $remote_path, $port = 21)
    {
        ...
        try
        {
            ...
            if(false !== $cid = SFtp::login($server, $user, $password, $port))
            {
                ...
                if(!SString::ends_with($local_path, '/'))
                {
                ...
                {
                    $upload = SFtp::upload_all($cid, $local_path, $remote_path);
                }
                ftp_close($cid); 
            }
        }
            ...
            while($file = $d->read())
            {
                ...
                if ($file != "." && $file != "..")
                {
                    ...
                    if (is_dir($local_dir . DIRECTORY_SEPARATOR . $file))
                    {
                        # Upload directory
                        # Recursive part
                        if(SFtp::upload_all($cid, $local_dir . DIRECTORY_SEPARATOR . $file, $remote_dir . DIRECTORY_SEPARATOR . $file))
                        {
                            $uploaded++;
        ...
        try
        {
            if(is_dir($local_dir) && is_writable($local_dir))
            {
                if(false !== $cid = SFtp::login($server, $user, $password, $port))
                {
                    ...
                    if(!SString::ends_with($remote_dir, '/'))
                    {
                    ...
                    $downloaded = SFtp::download_all($cid, $remote_dir, $local_dir); 
                    ftp_close($cid); 
                }
            }
            else
                ...
                if($files!==false)
                {
                    ...
                    foreach ($files as $file)
                    {
                        ...
                        if ($file != "." && $file != "..")
                        {
                            ...
                            if (SFtp::is_dir($file, $cid))// $remote_dir . DIRECTORY_SEPARATOR .
                            {                                
                                # Create directory on local filesystem
                                mkdir($local_dir . DIRECTORY_SEPARATOR . basename($file));
                                
                                # Recursive part 
                                if(SFtp::download_all($cid, $file, $local_dir . DIRECTORY_SEPARATOR . basename($file)))
                                {
                                    $downloaded++;
                                }
                            }
    ...
    public static function upload($server, $user, $password, $local_file, $remote_file, $port = 21, $mode = FTP_BINARY, $timeout = 1000)
    {
        ...
        {
            error_log("SFtp::upload_file : " . $e->getMessage());
        }
        return $uploaded;
    }
    /**
